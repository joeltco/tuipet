...
echo "    static const config_animation_entry_t ${ASSETS_PREFIX_LOWER}_animation_table[] CONFIG_ENTRIES_TABLE_SECTION = {" >> "$OUTPUT_FILE_5"
echo -e "        ${OUTPUT_FILE_5_ANIMATION_TABLE}" >> "$OUTPUT_FILE_5"
echo '    };' >> "$OUTPUT_FILE_5"
if [[ -n $ALT ]]; then
  echo -e "${OUTPUT_FILE_5_ALT_ANIMATIONS}\n" >> "$OUTPUT_FILE_5"
  echo "    static const config_animation_entry_t ${ASSETS_PREFIX_LOWER}_alt_animation_table[] CONFIG_ENTRIES_TABLE_SECTION = {" >> "$OUTPUT_FILE_5"
  echo -e "        ${OUTPUT_FILE_5_ALT_ANIMATION_TABLE}" >> "$OUTPUT_FILE_5"
  echo '    };' >> "$OUTPUT_FILE_5"
  echo "    static const size_t ${ASSETS_PREFIX_LOWER}_alt_animation_table_size CONFIG_ENTRIES_TABLE_SECTION = LEN_ARRAY(${ASSETS_PREFIX_LOWER}_animation_table);" >> "$OUTPUT_FILE_5"
fi
