#!/usr/bin/env python3
"""Build corpus/db/pen.json — unified Pendulum ORIGINAL (1998) Ver.1-5 database. Shares the 5 classic version evo .md with pen20; pen has the original longer stage timers + the 127-sprite original roster (no 20th additions).

Conditions from humulos /pen/ per-version (Nature Spirits / Deep Savers / Nightmare
...
SPR = os.path.join(CORPUS, "fan/wayland-vpets/assets/pen")

STAGE_TIMER = {"Baby I": "1 h", "Baby II": "16 h", "Child": "20 h",
               "Adult": "60 h", "Perfect": "68 h", "Ultimate": None, "Egg": "1 min"}  # pen = original, longer timers
...
def rec(name, stage=None, attr=None):
    ...
    if not r:
        r = records[norm(name)] = {"id": norm(name), "name": name, "stage": stage, "attribute": attr,
            "devices": {"pen": {"versions": [], "stage_time": STAGE_TIMER.get(stage), "evolves_to": []}},
            "sprite": find_sprite(name), "sources": ["humulos:pen20"]}
    if stage and not r["stage"]:
        r["stage"] = stage; r["devices"]["pen"]["stage_time"] = STAGE_TIMER.get(stage)
...
for k, path in SPI.items():
    if k not in records:
        records[k] = {"id": k, "name": os.path.basename(path)[:-4], "stage": None, "attribute": None,
            "devices": {"pen": {"versions": [], "stage_time": None, "evolves_to": []}},
...
withevo = sum(1 for r in records.values() if r["devices"]["pen"]["evolves_to"])
out = {"_meta": {"device": "pen", "approximate": True, "count": len(records),
        "source": "humulos /pen/ per-version (NS/DS/NSo/WG/ME) + wayland sprites/timers",
        "with_evolutions": withevo,
        "derived_note": "pen.json is DERIVED from pen20 classic-version evo data (the 5 versions are largely shared) with pen ORIGINAL timers + pen 127-sprite roster. A separate clean humulos /pen/ per-version pull would refine fidelity (pen has no 20th additions; some pen20 evo targets here may be 20th-only and show as sprite_gaps). 5 CLASSIC versions captured (Nature Spirits/Deep Savers/Nightmare Soldiers/Wind Guardians/Metal Empire). pen20's 20th-anniversary BONUS lines (Guilmon/V-mon/Terriermon/DORUmon/Ryudamon/etc.) NOT yet pulled -> addition_pending. Minor extraction noise: a few branch targets dropped (see device._unresolved), Mugendra=Mugendramon. JOGRESS is the key mechanic (jogress_attrs/jogress_partner/jogress_partners).",
        "addition_pending_count": len(pending), "sprite_gaps": no_sprite},
       "digimon": [records[k] for k in sorted(records)]}
with open(os.path.join(HERE, "pen.json"), "w") as fh: json.dump(out, fh, indent=1, ensure_ascii=False)
print(f"wrote pen.json: {len(records)} mons | with evolutions {withevo} | addition_pending {len(pending)} | sprite_gaps {len(no_sprite)}")
