...
def test_only_a_conquered_zone_is_a_veteran_road():
    z = _wild_zone()
    zi = adventure.zone_index(z)
    assert not Adventure(_pet(prog=zi), zone=z).replay      # the frontier: fresh
    assert Adventure(_pet(prog=zi + 1), zone=z).replay      # conquered: veteran
...
def test_veteran_foes_carry_the_trained_side_and_the_originals_stay_clean():
    z = _wild_zone()
    zi = adventure.zone_index(z)
    a = Adventure(_pet(prog=zi + 1), zone=z)
    b = a.boss
    ...
    fresh = Adventure(_pet(prog=zi), zone=z)
...
def test_veteran_bounties_pay_half_again():
    ...
    f = Adventure(_pet(prog=zi), zone=z)
    f.holiday = None
    f.chain(True)
